...
public class CamController : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        switch (mode)
        {
            ...
            case Mode.isZooming:
                ...
                if (wheel > 0)
                {
                    ...
                    {
                        desiredPosition = (vectorPoint - transform.position) / 5 + transform.position;
                    }
                }
                else if (wheel < 0)
                {
                    desiredPosition = -(targetRotation.position - transform.position) / 5 + transform.position;
                }
        ...
        transform.position = Vector3.ClampMagnitude(transform.position, 31f * 1000f);
    }
    ...
    private float LinePlaneIntersect(Vector3 u, Vector3 P0, Vector3 N, Vector3 D, ref Vector3 point)
    {
        ...
        if (Vector3.Dot(N, u) == 0) {
            s = Vector3.Dot(N, (D - P0)) / Vector3.Dot(N, u);
        }
        else
        {
            s = Vector3.Dot(N, (D - P0));
        }
        UnityEngine.Debug.Log("P0" + P0);
        UnityEngine.Debug.Log("D" + D);
        UnityEngine.Debug.Log("N" + N);
        UnityEngine.Debug.Log("u" + u);
        //float s = 1;
        point = P0 + s * u;
        return s;
    }
